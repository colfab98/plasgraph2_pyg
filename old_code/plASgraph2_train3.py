...
def main(config_file: 'YAML configuration file',
    ...
    if os.path.exists(cache_path):
        ...
        G = create_graph5.read_graph_set(file_prefix, train_file_list,
        ...
        all_graphs = create_graph5.Networkx_to_PyG(G, node_list, parameters)
    ...
    labels = torch.tensor([
        1 if G.nodes[node_id]["text_label"] == "plasmid" else 0
        for node_id in node_list
    ], dtype=torch.long).to(device)


    print("🔍 Label breakdown:")
    print(f"  Plasmid (1):      {(labels == 1).sum().item()}")
    print(f"  Non-Plasmid (0):  {(labels == 0).sum().item()}")
    ...
    for epoch in range(parameters["epochs"]):
        ...
        optimizer.zero_grad()
        outputs = model(data)
        train_loss = (criterion(outputs, labels) * masks_train).sum() / masks_train.sum()
        train_loss.backward()
        optimizer.step()
        ...
        with torch.no_grad():
            ...
            val_loss = (criterion(val_outputs, labels) * masks_validate).sum() / masks_validate.sum()
            ...
            y_true = labels_val_masked.cpu().numpy()
            y_probs = probs_val_masked.cpu().numpy()
            ...
            metric_auroc.update(probs_stacked, labels_val_masked)
