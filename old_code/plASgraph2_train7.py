...
def main(config_file: 'YAML configuration file',
    ...
    dataset_root_dir = os.path.join(model_output_dir, "processed_graph_data")
    os.makedirs(dataset_root_dir, exist_ok=True)

    dataset = create_graph8.ReadGraphSet(
        root=dataset_root_dir,
        file_prefix=file_prefix,
        file_list=train_file_list,
        minimum_contig_length=parameters['minimum_contig_length'],
        parameters=parameters,
        read_labels=True # Assuming you want to read labels for training
    )

    print(f"DEBUG: Before data = dataset[0]. Type of 'dataset': {type(dataset)}")

        # --- Start checking dataset object ---
    print("\n--- CHECKING DATASET OBJECT ---")
    print(f"Type of dataset: {type(dataset)}")

    if hasattr(dataset, 'data'):
        print(f"  dataset.data exists.")
        print(f"  Type of dataset.data: {type(dataset.data)}")
        if isinstance(dataset.data, torch_geometric.data.Data):
            print(f"  dataset.data is a PyG Data object.")
            print(f"  Shape of dataset.data.x: {dataset.data.x.shape}")
            print(f"  Number of nodes (from dataset.data): {dataset.data.num_nodes}")
            print(f"  Number of edges (from dataset.data): {dataset.data.num_edges}")
        else:
            print("  dataset.data is NOT a PyG Data object.")
            print(f"  Value of dataset.data: {dataset.data}")
    else:
        print("  dataset.data attribute DOES NOT exist.")

    # You can also try to access by index, even if it usually returns None for single graphs in InMemoryDataset
    try:
        indexed_data = dataset[0]
        print(f"  dataset[0] returned: {type(indexed_data)}")
        if indexed_data is None:
            print("  Warning: dataset[0] returned None.")
        elif hasattr(indexed_data, 'x'):
            print(f"  Shape of dataset[0].x: {indexed_data.x.shape}")
    except Exception as e:
        print(f"  Error accessing dataset[0]: {e}")

    print("--- FINISHED CHECKING DATASET OBJECT ---\n")

    data = dataset[0]

    print(f"DEBUG: After data = dataset[0]. Type of 'data': {type(data)}")

    if gml_file is not None:
        if hasattr(dataset, 'G'): # Check if the attribute exists as a safeguard
            nx.write_gml(dataset.G, path=gml_file)
        else:
            print("Warning: GML file not written. NetworkX graph 'G' was not found in the dataset object. Ensure ReadGraphSet's process method correctly sets self.G.")
    ...
    labels = torch.tensor([
        [1, 0] if G.nodes[node_id]["text_label"] == "plasmid" else
        [0, 1] if G.nodes[node_id]["text_label"] == "chromosome" else
        [0, 0]
        for node_id in node_list
    ...
    for epoch in range(parameters["epochs"]):
        ...
        with torch.no_grad():
            ...
            probs = torch.softmax(val_outputs, dim=1)[:, 1]  # Prob for class 1 (plasmid)


            
           # Step 1: Apply validation mask
            labels_val_masked = labels[masks_validate.bool()]
            probs_val_masked = probs[masks_validate.bool()]

            # Step 2: Further mask out unlabeled/ambiguous nodes ([0, 0] → sum == 0)
            valid_eval_mask = labels_val_masked.sum(dim=1) != 0

            # Step 3: Keep only valid nodes for evaluation
            labels_eval = labels_val_masked[valid_eval_mask]
            probs_eval = probs_val_masked[valid_eval_mask]

            # Step 4: Convert to numpy arrays for sklearn
            y_true = labels_eval.argmax(dim=1).cpu().numpy()  # 0 = chromosome, 1 = plasmid
            y_probs = probs_eval.cpu().numpy()                # probability of class 1 (plasmid)
