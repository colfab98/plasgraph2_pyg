...
def fix_gradients(config_params, model: torch.nn.Module):
    ...
    # Access gradient_clipping value safely, defaulting to 0.0 (no clipping) if not defined
    # Assuming 'parameters' is the config object from config.py
...
def main(config_file: 'YAML configuration file',
    ...
    for epoch in range(parameters["epochs"]):
        ...
        fix_gradients(parameters, model)
    ...
    plt.grid(True)
    plt.ylim(0, 1)
    plt.savefig(os.path.join(model_output_dir, "loss_over_epochs.png"))
    plt.close() # Close the plot to free memory
    print(f"Loss plot saved to {os.path.join(model_output_dir, 'loss_over_epochs.png')}")
    ...
    with torch.no_grad():
        ...
        probs_chromosome_val_masked = probs_chromosome_final[masks_validate.bool()] 
