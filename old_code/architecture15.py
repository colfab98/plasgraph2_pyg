...
class GGNNConv(MessagePassing):
    def __init__(self, in_channels, out_channels, activation='relu', dropout_rate=0.0):
        ...
        # Edge Gate layer
        self.lin_edge_gate = nn.Linear(in_channels * 2, 1)
    ...
    def message(self, x_i, x_j, norm): # x_i: target, x_j: source
        # Compute edge gate
        edge_gate_input = torch.cat([x_i, x_j], dim=-1)
        edge_gate_logit = self.lin_edge_gate(edge_gate_input)
