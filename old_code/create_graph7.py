...
class ReadGraphSet(InMemoryDataset):
    def __init__(self, root, file_prefix, file_list, minimum_contig_length, parameters, read_labels=True):
        ...
        print(f"DEBUG (ReadGraphSet.__init__): Initializing InMemoryDataset for root: {root}")

        super().__init__(root)

        print(f"DEBUG (ReadGraphSet.__init__): After super().__init__")
        print(f"DEBUG (ReadGraphSet.__init__): self.processed_paths[0]: {self.processed_paths[0]}")
        print(f"DEBUG (ReadGraphSet.__init__): Does processed file exist? {os.path.exists(self.processed_paths[0])}")
        print(f"DEBUG (ReadGraphSet.__init__): self.data is None: {self.data is None}")
        if self.data is not None:
            print(f"DEBUG (ReadGraphSet.__init__): Type of self.data: {type(self.data)}")
            if hasattr(self.data, 'x'):
                print(f"DEBUG (ReadGraphSet.__init__): Shape of self.data.x: {self.data.x.shape}")
        else:
            print("DEBUG (ReadGraphSet.__init__): self.data was not loaded by super().__init__.")
    ...
    def extract_features_np(self, node_id, features):
        return [self.G.nodes[node_id][f] for f in features]
        
    def extract_y_np(self):
        label_features = ["plasmid_label", "chrom_label"]
        y_np = np.array(
            [self.extract_features_np(node_id, label_features) for node_id in self.node_list]
        )
        if getattr(self.parameters, "loss_function", None) == "squaredhinge":
            y_np = y_np * 2 - 1
    ...
    def process(self):
        ...
        self.G = read_graph_set(self.file_prefix, self.file_list,
                                self.parameters['minimum_contig_length'], self.read_labels)
        print(f"DEBUG: Output of read_graph_set: {type(self.G)}")
        print(f"DEBUG: Number of nodes in self.G: {self.G.number_of_nodes()}")
        print(f"DEBUG: Number of edges in self.G: {self.G.number_of_edges()}")
        
        self.node_list = list(self.G.nodes)
        print(f"Number of nodes in NetworkX graph (self.G): {len(self.node_list)}")

        features_list = self.parameters['features']
        x_np = np.array(
            [self.extract_features_np(node_id, features_list) for node_id in self.node_list],
            dtype=np.float32 
        )
        x = torch.tensor(x_np, dtype=torch.float)
        print(f"Shape of node features (x): {x.shape}")

        y_np = self.extract_y_np() 
        y = torch.tensor(y_np, dtype=torch.float) 
        print(f"Shape of labels (y): {y.shape}")

       
        adj_matrix_scipy = nx.adjacency_matrix(self.G, nodelist=self.node_list, dtype=np.float32)
        edge_index, edge_attr = from_scipy_sparse_matrix(adj_matrix_scipy)
        ...
        if self.pre_filter is not None and not self.pre_filter(pyg_data):
            data_list = []
            print("INFO: Data filtered out by pre_filter.")
        else:
            data_list = [pyg_data]
            print(f"Number of Data objects in data_list before pre_transform: {len(data_list)}")


        # if self.pre_transform is not None:
        #     data_list = [self.pre_transform(data_item) for data_item in data_list]
        # print(f"Number of Data objects in data_list after pre_transform: {len(data_list)}")
        
        processed_data, slices = self.collate([data_list])
        print(f"Processed data after collate: {processed_data}")
        print(f"Slices after collate: {slices}")

        torch.save((processed_data, slices), self.processed_paths[0])
        print(f"--- Finished ReadGraphSet.process(). Data saved to: {self.processed_paths[0]} ---")

        self.data, self.slices = processed_data, slices
        print(f"DEBUG (ReadGraphSet.process): self.data assigned: {self.data is not None}")
