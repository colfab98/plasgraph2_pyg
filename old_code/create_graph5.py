...
from torch_geometric.utils import from_scipy_sparse_matrix
import old_code.create_graph5 as create_graph5  # uses read_graph_set and helpers

class PlasGraphDataset(InMemoryDataset):
    def __init__(self, root, train_file_list, file_prefix, parameters,
                 transform=None, pre_transform=None, pre_filter=None, gml_file=None):
    ...
    def _extract_features_np(self, node_id, features): 
            return [self.nx_graph.nodes[node_id][f] for f in features]
        
    def _extract_y_np(self): 
    ...
    def process(self):
        ...
        self.nx_graph = G # Store for access by helper methods if needed during processing
        self.node_order = list(G)
        
        current_node_order = self.node_order

        if self.gml_file is not None:
            nx.write_gml(G, self.gml_file)


        features_list = self.parameters['features']

        x_np = np.array(
            [self._extract_features_np(node_id, features_list) for node_id in current_node_order],
            dtype=np.float32 
        )
        x = torch.tensor(x_np, dtype=torch.float)

        y_np = self._extract_y_np() 
        y = torch.tensor(y_np, dtype=torch.float) 

        adj_matrix_scipy = nx.adjacency_matrix(G, nodelist=current_node_order, dtype=np.float32)
